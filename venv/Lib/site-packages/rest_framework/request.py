...
class Request(object):
    ...
    def _parse(self):
        ...
        #选择解析器，parsers就是parser_classes = [JSONParser, FormParser]
        # self,请求对象，self.content-type
    ...
    def _authenticate(self):
        ...
        in turn.
        """
        # authenticators = [BasicAuthentication对象,]
        # 循环认证类的所有对象
        for authenticator in self.authenticators:
            try:
                # 执行认证类的authenticate的方法，
                # 1.如果authenticate抛异常下面捕获，self._not_authenticated()执行
                # 2.有返回值，必须是元组：(request.user,request.auth)，循环中止
                # 3.返回None,走下一次循环，表示当前不认证，进行下一次认证
    ...
    def _not_authenticated(self):
        ...
        if api_settings.UNAUTHENTICATED_USER:
            self.user = api_settings.UNAUTHENTICATED_USER() # AnonymousUser(默认值：匿名用户)
        else:
            self.user = None
